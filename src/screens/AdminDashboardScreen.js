      ...
      <View style={styles.tabBar}>
        ...
        <TouchableOpacity
          ...
          <Text style={[styles.tabText, activeTab === 'calls' && styles.tabTextActive]}>
            📞 Call Requests {callRequests.length > 0 ? `(${callRequests.length})` : ''}
          </Text>
        </TouchableOpacity>
      </View>
          ...
          convoLoading ? (
            ...
            conversations.map(convo => {
              ...
              const lastTime = convo.lastTimestamp
                ? convo.lastTimestamp.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
                : '—';
              const userMsgs = convo.messages.filter(m => m.sender === 'user').length;

              return (
                <View key={convo.userId} style={styles.convoCard}>
                  <TouchableOpacity
                    style={styles.convoHeader}
                    onPress={() => setExpandedUser(isOpen ? null : convo.userId)}>
                    ...
                    <View style={styles.convoHeaderInfo}>
                      ...
                      <Text style={styles.convoMeta}>
                        {userMsgs} message{userMsgs !== 1 ? 's' : ''} · Last active {lastTime}
                      </Text>
                      {!isOpen && lastMsg && (
          ...
          callLoading ? (
            ...
            callRequests.map(call => {
              ...
              const date     = call.createdAt?.toDate
                ...
                : '—';
              const typeLabel = call.callType === 'legal' ? '🏛️ Legal Consultation' : '⚖️ Advocate Call';
                ...
                <View key={call.id} style={styles.reportCard}>
                  ...
                  {isOpen && (
                    <View style={styles.reportDetail}>
                      ...
                      <Text style={styles.detailText}>{call.preferredDate || '—'}</Text>

                      <Text style={styles.detailLabel}>Preferred Time</Text>
                      <Text style={styles.detailText}>{call.timeSlot || '—'}</Text>
                      ...
                      {call.reportId ? (
                        <>
                          ...
                          <Text style={styles.detailText}>{call.reportType || call.reportId}</Text>
                        </>
                      ) : null}

                      {/* Contact user */}
                      <Text style={[styles.detailLabel, { marginTop: 16 }]}>Contact User</Text>
                      <View style={styles.contactRow}>
                        {call.userEmail ? (
                          <TouchableOpacity
                            ...
                            <Text style={styles.contactBtnText}>📧 Email User</Text>
                          </TouchableOpacity>
                        ) : null}
                        ...
                        {call.phone ? (
                          <TouchableOpacity
                            ...
                            <Text style={[styles.contactBtnText, styles.contactBtnSmsText]}>💬 SMS User</Text>
                          </TouchableOpacity>
                        ) : null}
                      </View>

                      <Text style={[styles.detailLabel, { marginTop: 16 }]}>Update Status</Text>
                        ...
                        {STATUS_OPTIONS.map(s => (
                          <TouchableOpacity
                            ...
                            style={[
                              ...
                              status === s && { backgroundColor: STATUS_COLORS[s].bg },
                            ]}
                            ...
                            {updatingCall === call.id ? (
                              ...
                              <Text style={[
                                ...
                                status === s && { color: STATUS_COLORS[s].text, fontWeight: 'bold' },
                              ]}>
                                {STATUS_LABELS[s]}
                              </Text>
                            )}
        ...
        <View style={[styles.statsRow, isWide && styles.statsRowWide]}>
          {[
            ...
            <View key={s.key} style={styles.statCard}>
              <Text style={[styles.statNumber, { color: s.color }]}>{counts[s.key]}</Text>
                  ...
                  <View style={styles.reportDetail}>
                    ...
                    {report.witnesses && report.witnesses !== 'No' ? (
                      <>
                        ...
                        <Text style={styles.detailText}>{report.witnessNames || report.witnesses}</Text>
                      </>
                    ) : null}
                    ...
                    <Text style={[styles.detailLabel, { marginTop: 16 }]}>Contact User</Text>
                    {report.userEmail ? (
                      <Text style={styles.contactInfoText}>✉️ {report.userEmail}</Text>
                    ) : null}
                    {report.contactInfo ? (
                      <Text style={styles.contactInfoText}>📱 {report.contactInfo}</Text>
                    ) : null}
                    <View style={styles.contactRow}>
                      {report.userEmail ? (
                        <TouchableOpacity
                        ...
                        <Text style={styles.noContactText}>No email on file — ask user to resubmit</Text>
                      )}
                      {report.contactInfo ? (
                        <TouchableOpacity
                          style={[styles.contactBtn, styles.contactBtnSms]}
...
const styles = StyleSheet.create({
  ...
  heroTitle: { fontSize: 30, fontWeight: 'bold', color: '#fff', marginBottom: 8 },
  ...
  detailLabel: { fontSize: 12, fontWeight: 'bold', color: '#888', marginBottom: 4, marginTop: 10, textTransform: 'uppercase', letterSpacing: 0.5 },
  detailText:  { fontSize: 14, color: '#333', lineHeight: 21 },

  statusButtons: { flexDirection: 'row', gap: 8, flexWrap: 'wrap', marginTop: 8 },
  statusBtn: {
    borderWidth: 1,
    borderColor: '#ddd',
    borderRadius: 6,
  ...
  contactBtn:       { flexDirection: 'row', alignItems: 'center', backgroundColor: '#E3F2FD', borderRadius: 8, paddingHorizontal: 14, paddingVertical: 10, borderWidth: 1, borderColor: '#90CAF9' },
  ...
  noContactText:    { fontSize: 13, color: '#aaa', fontStyle: 'italic', marginTop: 4 },

  empty: { paddingVertical: 60, alignItems: 'center' },
  emptyText: { fontSize: 16, color: '#999' },
  emptySubText: { fontSize: 13, color: '#bbb', marginTop: 6, textAlign: 'center', maxWidth: 300 },
